...
/* The pinned imageCubeArray object-management CTS leaf creates one RGBA8
 * cube-compatible image with both sampled and colour-attachment usage. Keep
 * this exception tied to that exact resource shape; it does not widen the
 * ordinary sampled/attachment usage table or imply that tiled attachments can
 * be sampled by the runtime descriptor path. */
static inline int ps5vk_graphics_image_usage_with_flags(
    VkFormat format, VkImageType type, VkImageTiling tiling,
    VkImageUsageFlags usage, VkImageCreateFlags flags)
{
    if (ps5vk_graphics_image_usage(format, usage)) return 1;
    return format == VK_FORMAT_R8G8B8A8_UNORM &&
        type == VK_IMAGE_TYPE_2D && tiling == VK_IMAGE_TILING_OPTIMAL &&
        flags == VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT &&
        usage == (VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) &&
        ps5vk_texture_format_has(format, PS5VK_FORMAT_CAP_SAMPLED_IMAGE) &&
