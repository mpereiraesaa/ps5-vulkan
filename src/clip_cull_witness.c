...
static int covers(unsigned witness_case,double ndc_x,double ndc_y)
{
    ...
    default:
        /* Cull distances discard the whole primitive, and this witness draws
         * one primitive whose vertices are never all on the positive side. */
        return 0;
    }
}
...
void ps5vk_clip_cull_witness_expected(unsigned x,unsigned y,unsigned extent,uint8_t rgba[4])
{
    ...
    const double v=1.0-((double)y+0.5)/(double)extent;
    rgba[0]=unorm8(u);
    rgba[1]=unorm8(v);
    rgba[2]=unorm8(0.5);
    rgba[3]=255u;
