    ...
    const struct ps5vk_operation *op,uint32_t out[4])
{
    ...
    if(b->binding || b->inputRate!=VK_VERTEX_INPUT_RATE_VERTEX || !b->stride || b->stride%4 ||
    ...
    for(uint32_t i=0;i<key->vertex_attribute_count;++i) {
        ...
        if(a->binding || a->location>=32 || a->offset%4 || a->offset>b->stride || size>b->stride-a->offset)return VK_ERROR_UNKNOWN;
    ...
    if(!(op->type==PS5VK_DRAW_INDEXED?op->index_count:op->vertex_count) || !op->instance_count)return VK_SUCCESS;
    ...
    uint32_t words[4];
    if(ps5vk_vertex_descriptor(words,(uintptr_t)address,bytes,b->stride,extent))return VK_ERROR_UNKNOWN;
    ...
    if(op->type==PS5VK_DRAW && (uint64_t)op->first_vertex+op->vertex_count>words[2])return VK_ERROR_UNKNOWN;
