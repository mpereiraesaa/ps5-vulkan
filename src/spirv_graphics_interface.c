...
static int reflect(const struct ps5vk_graphics_module_key *m,unsigned model,struct interface *out)
{
    ...
    for(unsigned i=1;i<bound;++i) {
        ...
        if(d->builtin!=~0u) {
             ...
             * VertexIndex (42) comes from the geometry path, and BaseVertex
             * (4424), BaseInstance (4425) and DrawIndex (4426) come from the
             * user-SGPR block the draw emitter fills from the recorded draw.
             * They are not vertex attributes, so they never enter the input
             * map, and anything else stays refused. */
            if(d->location!=~0u || model!=0 || d->storage!=1 ||
               (d->builtin!=42 && d->builtin!=4424 && d->builtin!=4425 &&
                d->builtin!=4426) ||
