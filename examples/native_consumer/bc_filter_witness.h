 ...
 * Public-SDK-only BC linear filtering witness, one format per executable.
 * Included after CHECK/REQUIRE, ps5log and the generated shader arrays.
 */
#include "bc_filter_shaders.h"
#include "bc_filter_data.h"

enum {
    BC_FILTER_LAYER_COUNT = 1, BC_FILTER_STORAGE_LAYERS = 1,
    BC_FILTER_FACE_EXTENT = 8,
    BC_FILTER_TARGET_WIDTH = 64, BC_FILTER_TARGET_HEIGHT = 64,
    BC_FILTER_PIXEL_BYTES = 4, BC_FILTER_FACE_BYTES = sizeof(bc_filter_blocks),
};

static VkDeviceMemory bc_filter_allocate(VkDevice device, VkDeviceSize size)
{
...
static void run_bc_filter_witness(VkPhysicalDevice physical, VkDevice device,
                                   VkQueue queue)
{
    ps5log_printf(PS5LOG_MARK,
        "PS5VK_CONSUMER_BC_FILTER_START format=%u image=8x8 target=64x64 filter=linear"
        " input_sha256=%s reference_sha256=%s",
        (unsigned)BC_FILTER_FORMAT, BC_FILTER_INPUT_SHA256, BC_FILTER_REFERENCE_SHA256);

    VkFormatProperties format_properties = {0};
    vkGetPhysicalDeviceFormatProperties(physical, BC_FILTER_FORMAT,
                                        &format_properties);
    ...
    REQUIRE((format_properties.optimalTilingFeatures & sampled_required) == sampled_required,
            "BC linear-filter color format reports sampled and transfer-destination support");
    ...
    REQUIRE((format_properties.optimalTilingFeatures & target_required) == target_required,
            "BC linear-filter witness target reports attachment and transfer-source support");
    ...
    VkImageCreateInfo cube_info = {
        ...
        .format = BC_FILTER_FORMAT,
        .extent = {BC_FILTER_FACE_EXTENT, BC_FILTER_FACE_EXTENT, 1},
        .mipLevels = 1,
        .arrayLayers = BC_FILTER_STORAGE_LAYERS,
    ...
    cube_memory = bc_filter_allocate(device, cube_requirements.size);
    ...
    VkBufferCreateInfo upload_info = {
        ...
        .size = BC_FILTER_STORAGE_LAYERS * BC_FILTER_FACE_BYTES,
    ...
    upload_memory = bc_filter_allocate(device, upload_requirements.size);
    ...
    CHECK(vkMapMemory(device, upload_memory, 0, VK_WHOLE_SIZE, 0,
                      (void **)&upload_bytes));
    memcpy(upload_bytes, bc_filter_blocks, sizeof(bc_filter_blocks));
    ...
    VkImageCreateInfo target_info = {
        ...
        .extent = {BC_FILTER_TARGET_WIDTH, BC_FILTER_TARGET_HEIGHT, 1},
        .mipLevels = 1,
        .arrayLayers = 1,
        .samples = VK_SAMPLE_COUNT_1_BIT,
        .tiling = VK_IMAGE_TILING_OPTIMAL,
    ...
    target_memory = bc_filter_allocate(device, target_requirements.size);
    ...
    VkBufferCreateInfo readback_info = {
        .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
        .size = BC_FILTER_TARGET_WIDTH * BC_FILTER_TARGET_HEIGHT *
                BC_FILTER_PIXEL_BYTES,
    ...
    readback_memory = bc_filter_allocate(device, readback_requirements.size);
    ...
    VkFramebufferCreateInfo framebuffer_info = {
        ...
        .width = BC_FILTER_TARGET_WIDTH,
        .height = BC_FILTER_TARGET_HEIGHT,
        .layers = 1,
    };
    ...
    VkShaderModuleCreateInfo shader_info = {
        ...
        .codeSize = sizeof(bc_filter_vert_spirv),
        .pCode = bc_filter_vert_spirv,
    };
    CHECK(vkCreateShaderModule(device, &shader_info, NULL, &vertex_module));
    shader_info.codeSize = sizeof(bc_filter_frag_spirv);
    shader_info.pCode = bc_filter_frag_spirv;
    ...
    VkViewport viewport = {0.0f, 0.0f, BC_FILTER_TARGET_WIDTH,
                           BC_FILTER_TARGET_HEIGHT, 0.0f, 1.0f};
    VkRect2D scissor = {{0, 0}, {BC_FILTER_TARGET_WIDTH, BC_FILTER_TARGET_HEIGHT}};
    ...
    VkImageMemoryBarrier cube_barrier = {
        ...
        .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                             BC_FILTER_STORAGE_LAYERS},
    };
    vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                         VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL,
                         1, &cube_barrier);
    VkBufferImageCopy face_copies[BC_FILTER_STORAGE_LAYERS];
    for (unsigned layer = 0; layer < BC_FILTER_STORAGE_LAYERS; ++layer) {
        face_copies[layer] = (VkBufferImageCopy){
            .bufferOffset = layer * BC_FILTER_FACE_BYTES,
            .imageSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, layer, 1},
            .imageOffset = {0, 0, 0},
            .imageExtent = {BC_FILTER_FACE_EXTENT, BC_FILTER_FACE_EXTENT, 1},
        };
    }
    vkCmdCopyBufferToImage(command_buffer, upload_buffer, cube_image,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           BC_FILTER_STORAGE_LAYERS, face_copies);
    ...
    VkRenderPassBeginInfo render_begin = {
        ...
        .renderArea = {{0, 0}, {BC_FILTER_TARGET_WIDTH, BC_FILTER_TARGET_HEIGHT}},
        .clearValueCount = 1,
        .pClearValues = &clear,
    };
    ...
    VkBufferImageCopy target_copy = {
        ...
        .imageExtent = {BC_FILTER_TARGET_WIDTH, BC_FILTER_TARGET_HEIGHT, 1},
    };
    ...
    REQUIRE(wait_result == VK_SUCCESS,
            "BC linear-filter witness GPU work completes within its bounded fence");
    ...
    for (unsigned i = 0; i < BC_FILTER_TARGET_WIDTH * BC_FILTER_TARGET_HEIGHT; ++i) {
        unsigned bad = 0;
        for (unsigned c = 0; c < 4; ++c) {
            int delta = (int)pixels[i * 4 + c] - (int)bc_filter_expected[i * 4 + c];
            unsigned error = (unsigned)(delta < 0 ? -delta : delta);
            if (error > max_error) max_error = error;
            bad |= error > BC_FILTER_TOLERANCE;
        }
        if (bad && mismatches < 4)
            ps5log_printf(PS5LOG_INFO,
                "PS5VK_CONSUMER_BC_FILTER_PIXEL index=%u actual=%u,%u,%u,%u expected=%u,%u,%u,%u",
                i, pixels[i*4], pixels[i*4+1], pixels[i*4+2], pixels[i*4+3],
                bc_filter_expected[i*4], bc_filter_expected[i*4+1],
                bc_filter_expected[i*4+2], bc_filter_expected[i*4+3]);
        mismatches += bad;
    }
    ps5log_printf(PS5LOG_MARK,
        "PS5VK_CONSUMER_BC_FILTER_RESULT pixels=4096 mismatches=%u max_error=%u tolerance=%u",
        mismatches, max_error, BC_FILTER_TOLERANCE);
    REQUIRE(mismatches == 0, "GPU BC linear samples match the independent CPU reference");
