...
#include "ps5log.h"
#include "t08_subgroup_basic_shader.h"
#define WITNESS_SPIRV t08_subgroup_basic_spirv
#define WITNESS_START "T08_SUBGROUP_BASIC_START"
#define WITNESS_RESULT "T08_SUBGROUP_BASIC_RESULT"
#define WITNESS_RETIRED "T08_SUBGROUP_BASIC_RETIRED"
#define WITNESS_FAILURE "T08_SUBGROUP_BASIC_FAILURE"

#include <stdint.h>
#include <time.h>
#include <unistd.h>

/* Two 16x4 workgroups: four wave32 subgroups, seven words per invocation
 * (SubgroupSize, SubgroupInvocationID, SubgroupID, NumSubgroups, uniform
 * Elect, odd-lane Elect, subgroup-barrier shared exchange). */
enum { GROUPS = 2, INVOCATIONS = 128, FIELDS = 7, OUTPUTS = INVOCATIONS * FIELDS,
       OUTPUT_OFFSET = 16, WORDS = OUTPUT_OFFSET + OUTPUTS + 8 };
static const uint32_t sentinel = UINT32_C(0xdeadbeef);
static const uint32_t salts[GROUPS] = {UINT32_C(0x1000), UINT32_C(0x2000)};

static uint32_t expected_word(uint32_t index)
{
    const uint32_t invocation = index / FIELDS, field = index % FIELDS;
    const uint32_t group = invocation / 64, local = invocation % 64;
    const uint32_t lane = local % 32;
    switch (field) {
    case 0: return 32;
    case 1: return lane;
    case 2: return local / 32;
    case 3: return 2;
    case 4: return lane == 0;
    case 5: return (lane & 1) ? lane == 1 : 2;
    default: return (local ^ 1u) * 3u + salts[group];
    }
}
...
#define REQUIRE(ok, reason) do { if (!(ok)) { \
    ...
    ps5log_printf(PS5LOG_MARK,
        WITNESS_START " groups=2 local=16x4 subgroups=4 fields=7 api=1.0");
    ...
    for (uint32_t i = 0; i < GROUPS; ++i)
        guards += words[i] != salts[i];
    for (uint32_t i = GROUPS; i < OUTPUT_OFFSET; ++i)
        guards += words[i] != sentinel;
    uint32_t first_bad = UINT32_MAX, first_value = 0;
    for (uint32_t i = 0; i < OUTPUTS; ++i) {
        uint32_t actual = words[OUTPUT_OFFSET + i];
        if (actual != expected_word(i) && first_bad == UINT32_MAX) {
            first_bad = i; first_value = actual;
        }
        mismatches += actual != expected_word(i);
        digest = digest_word(digest, actual);
    }
    if (first_bad != UINT32_MAX)
        ps5log_printf(PS5LOG_ERR, WITNESS_RESULT " first_mismatch invocation=%u field=%u "
            "actual=%u expected=%u", first_bad / FIELDS, first_bad % FIELDS,
            first_value, expected_word(first_bad));
...
int main(void)
{
    ...
    int failed = witness();
    ps5log_close(failed ? "t08-subgroup-basic-failed" : "t08-subgroup-basic-end");
    for (;;) sleep(1);
}
