#!/usr/bin/env python3
"""Build the bounded public-SDK 1 GiB device-memory witness executable.

The SDK is the ordinary staged graphics build; the witness allocates, binds,
GPU-writes and frees one 1 GiB allocation through the public API only."""
...
PROFILE = "dxvk-heap-1gib-public-sdk-witness"
GIB = 1 << 30
ITERATIONS = 3
REGION_BYTES = 65536
REGION_OFFSETS = (0, GIB // 2, GIB - REGION_BYTES)
HEADROOM_CHUNK_BYTES = 32 << 20
...
def seed_for(iteration: int, region: int) -> int:
    return 0x4EA90000 | (iteration << 4) | region


def expected_digest(seed: int) -> int:
    digest = 2166136261
    for index in range(REGION_BYTES // 4):
        value = ((index * 2654435761) & 0xffffffff) ^ seed
...
def main() -> None:
    ...
    build = ROOT / "build/dxvk-heap-witness"
    ...
    source = ROOT / "examples/dxvk_heap_witness/main.c"
    obj = build / "main.o"
    dep = build / "main.d"
    ...
    param.update(titleId="PPSA99994", conceptId="99994",
                 contentId="UP9000-PPSA99994_00-PS5VKHEAP0000001")
    param["localizedParameters"]["en-US"]["titleName"] = (
        "PS5 Vulkan 1 GiB Heap Witness")
    ...
    artifact = {
        "profile": PROFILE, "iterations": ITERATIONS,
        "region_bytes": REGION_BYTES, "region_offsets": list(REGION_OFFSETS),
        "headroom_chunk_bytes": HEADROOM_CHUNK_BYTES,
        "sdk_switches": {},
