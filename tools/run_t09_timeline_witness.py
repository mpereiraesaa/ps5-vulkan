#!/usr/bin/env python3
"""Run and strictly verify one bounded public-SDK timeline-semaphore witness."""
...
from build_t09_timeline_witness import (  # noqa: E402
    GATE_VALUE, INITIAL_VALUE, PHASE_VALUES, PROFILE, SEEDS, VALUES, expected_digest)
from run_consumer import close_and_confirm, control, running, wait_for_log  # noqa: E402

START = re.compile(r"T09_TIMELINE_WITNESS_START spec=(\d+) max_difference=(\d+) "
                   r"initial=(\d+) values=(\d+)")
DEFERRED = re.compile(r"T09_TIMELINE_WITNESS_DEFERRED submit=(\w+) counter=(\d+) "
                      r"wait_zero=(\w+) wait_20ms=(\w+) untouched=(\w+)")
RESULT = re.compile(r"T09_TIMELINE_WITNESS_RESULT phase=(\d) wait=(\w+) counter=(\d+) "
                    r"values=(\d+) mismatches=(\d+) guard_mismatches=(\d+) "
                    r"digest=([0-9a-f]{8})")
RETIRED = re.compile(r"T09_TIMELINE_WITNESS_RETIRED resources=(\w+)")
...
def verify(log: bytes, receipt: dict, artifact: dict) -> dict:
    if (artifact.get("profile") != PROFILE or artifact.get("values") != VALUES or
            ...
            artifact.get("seeds") != list(SEEDS)):
        raise ValueError("unexpected timeline witness artifact")
    ...
    if (len(start) != 1 or len(deferred) != 1 or len(results) != 2 or
            len(retired) != 1 or "T09_TIMELINE_WITNESS_FAILURE" in text):
        raise ValueError("missing, repeated or failed witness phase")
    spec, difference, initial, values = (int(field) for field in start[0])
    if spec < 1 or difference < 2147483647 or initial != INITIAL_VALUE or values != VALUES:
        raise ValueError("timeline exposure does not match the witness contract")
    # Nothing may be visible before the host signal: the submission returned,
    # the counter still held its initial value, both waits timed out and the
    # buffer still held only guard words.
    if deferred[0] != ("returned", str(INITIAL_VALUE), "timeout", "timeout", "yes"):
        raise ValueError("work or payload became visible before the host signal")
    for phase, (seed, value) in enumerate(zip(SEEDS, PHASE_VALUES), start=1):
        expected = (str(phase), "success", str(value), str(VALUES), "0", "0",
                    f"{expected_digest(seed):08x}")
        if results[phase - 1] != expected:
    ...
    order = [text.index(marker) for marker in (
        "T09_TIMELINE_WITNESS_START", "T09_TIMELINE_WITNESS_DEFERRED",
        "T09_TIMELINE_WITNESS_RESULT phase=1", "T09_TIMELINE_WITNESS_RESULT phase=2",
        "T09_TIMELINE_WITNESS_RETIRED")]
    ...
    return {
        ...
        "spec_version": spec,
        "max_timeline_semaphore_value_difference": difference,
