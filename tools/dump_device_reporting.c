...
/* The pinned DXVK 2.6.2 requirement profile, generated by
 * tools/derive_dxvk_profile.py: its X-macros name every core aggregate member
 * the profile requires, so the dump publishes exactly the values the native
 * capability probe observes (tools/check_dxvk_profile.py compares the two). */
#include "../examples/native_consumer/dxvk_v262_profile.h"
...
VkResult ps5vk_platform_query(struct ps5vk_platform *platform)
{
    ...
    /* Mirrors native/platform_ps5.c: the draw-parameter built-ins belong to
     * the graphics runtime path. */
    if (graphics_submit)
    ...
    /* Mirrors native/platform_ps5.c: maintenance4 (creation-description
     * memory requirements, LocalSizeId and maxBufferSize) on every profile. */
...
/* The Vulkan 1.1/1.2/1.3 aggregate structures, answered through the public
 * Features2/Properties2 entry points for every member the DXVK profile names.
 * An aggregate the reported version does not answer prints null. */
static void print_core_version_queries(FILE *out, uint32_t api_version)
{
    ...
    const int has12 = api_version >= VK_API_VERSION_1_2;
    const int has13 = api_version >= VK_API_VERSION_1_3;
    if (has12) {
        f11.pNext = &f12;
        f12.pNext = has13 ? (void *)&f13 : NULL;
        p11.pNext = &p12;
        p12.pNext = has13 ? (void *)&p13 : NULL;
        VkPhysicalDeviceFeatures2 features2 = {
            .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, .pNext = &f11};
        VkPhysicalDeviceProperties2 properties2 = {
            .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2, .pNext = &p11};
        vkGetPhysicalDeviceFeatures2KHR(dump_physical, &features2);
        vkGetPhysicalDeviceProperties2KHR(dump_physical, &properties2);
    }
    const char *sep;
#define DUMP_FEATURE(s, name, expected) \
    fprintf(out, "%s\"%s\": %s", sep, #name, (s).name ? "true" : "false"); sep = ", ";
#define DUMP_PROPERTY(s, name, expected) \
    fprintf(out, "%s\"%s\": %llu", sep, #name, (unsigned long long)(s).name); sep = ", ";
#define F11(name, expected) DUMP_FEATURE(f11, name, expected)
#define F12(name, expected) DUMP_FEATURE(f12, name, expected)
#define F13(name, expected) DUMP_FEATURE(f13, name, expected)
#define P11(name, expected) DUMP_PROPERTY(p11, name, expected)
#define P12(name, expected) DUMP_PROPERTY(p12, name, expected)
#define P13(name, expected) DUMP_PROPERTY(p13, name, expected)
#define DUMP_STRUCT(label, present, list, macro, last) \
    fprintf(out, "    \"%s\": ", label); \
    if (present) { sep = ""; fputc('{', out); list(macro) fputc('}', out); } \
    else fputs("null", out); \
    fputs(last ? "\n" : ",\n", out);
    fputs("  \"coreVersionQueries\": {\n", out);
    DUMP_STRUCT("VkPhysicalDeviceVulkan11Features", has12,
                DXVK262_FEATURES_VK_PHYSICAL_DEVICE_VULKAN11_FEATURES, F11, 0)
    DUMP_STRUCT("VkPhysicalDeviceVulkan12Features", has12,
                DXVK262_FEATURES_VK_PHYSICAL_DEVICE_VULKAN12_FEATURES, F12, 0)
    DUMP_STRUCT("VkPhysicalDeviceVulkan13Features", has13,
                DXVK262_FEATURES_VK_PHYSICAL_DEVICE_VULKAN13_FEATURES, F13, 0)
    DUMP_STRUCT("VkPhysicalDeviceVulkan11Properties", has12,
                DXVK262_PROPERTIES_VK_PHYSICAL_DEVICE_VULKAN11_PROPERTIES, P11, 0)
    DUMP_STRUCT("VkPhysicalDeviceVulkan12Properties", has12,
                DXVK262_PROPERTIES_VK_PHYSICAL_DEVICE_VULKAN12_PROPERTIES, P12, 0)
    DUMP_STRUCT("VkPhysicalDeviceVulkan13Properties", has13,
                DXVK262_PROPERTIES_VK_PHYSICAL_DEVICE_VULKAN13_PROPERTIES, P13, 1)
    fputs("  },\n", out);
#undef DUMP_STRUCT
#undef P13
#undef P12
...
int main(int argc, char **argv)
{
    ...
    print_core_version_queries(stdout, properties.apiVersion);
