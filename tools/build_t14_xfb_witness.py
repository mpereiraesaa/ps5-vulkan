...
CASES = ("inactive", "small", "order", "resume", "overflow", "streams")
SHADERS = {
    ...
    "t14_xfb_capture_geom_spirv": ("geom", "experiments/graphics/xfb_capture.geom"),
    "t14_xfb_streams_geom_spirv": ("geom", "experiments/graphics/xfb_streams.geom"),
}
# Shader, Geometry, TransformFeedback, GeometryStreams.
ALLOWED_CAPABILITIES = {1, 2, 53, 54}


def checked_spirv(payload: bytes) -> None:
    """A Vulkan-1.0 SPIR-V module using only the capture capabilities."""
    if len(payload) % 4:
    ...
    while index < len(words):
        ...
        index += size
    if not capabilities <= ALLOWED_CAPABILITIES:
        raise ValueError("witness shaders use capabilities outside the capture set")
...
def main() -> None:
    ...
    arrays = []
    shader_hashes = {}
    for name, (stage, relative) in SHADERS.items():
        target = build / f"{name}.spv"
        run(glslang, "-V", "-S", stage, "--target-env", "vulkan1.0", str(ROOT / relative),
            "-o", str(target))
    ...
    param.update(titleId="PPSA99994", conceptId="99994",
                 contentId="UP9000-PPSA99994_00-PS5VKXFBWITN0001")
    param["localizedParameters"]["en-US"]["titleName"] = "PS5 Vulkan Transform Feedback Witness"
