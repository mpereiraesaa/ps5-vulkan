#!/usr/bin/env python3
"""Build the bounded public-SDK buffer-device-address witness executable."""
...
def checked_spirv(payload: bytes) -> None:
    ...
    while index < len(words):
        ...
        index += size
    if ({1, 5347} != capabilities or memory_model != (5348, 1) or
            "SPV_KHR_physical_storage_buffer" not in extensions):
        raise ValueError("shader lacks the Vulkan-1.0 physical-address contract")
...
def main() -> None:
    ...
    build = ROOT / "build/t08-bda-witness"
    dist = ROOT / "dist-t08-bda-witness/PPSA99994"
    for directory in (build, dist / "sce_sys", dist / "sce_module"):
        directory.mkdir(parents=True, exist_ok=True)

    shader_source = ROOT / "experiments/compute/t08_bda_two_buffer.comp"
    shader_file = build / "two_buffer.spv"
    run(glslang, "-V", "--target-env", "vulkan1.0", str(shader_source),
        "-o", str(shader_file))
    shader = shader_file.read_bytes()
    checked_spirv(shader)
    (build / "t08_bda_shader.h").write_text(
    ...
    param.update(titleId="PPSA99994", conceptId="99994",
                 contentId="UP9000-PPSA99994_00-PS5VKBDA00000001")
    param["localizedParameters"]["en-US"]["titleName"] = (
        "PS5 Vulkan Buffer Address Witness")
    ...
    artifact = {
        "profile": "t08-bda-public-sdk-witness",
