...
# ImageQuery (50) only in the size-query module.
SHADERS = {
    "t13_access_spirv": ("experiments/compute/t13_robustness2_access.comp", {1, 46}),
    "t13_query_spirv": ("experiments/compute/t13_robustness2_query.comp", {1, 46, 50}),
}
...
def checked_spirv(payload: bytes, capabilities: set[int]) -> None:
    """A SPIR-V 1.0 GLCompute module with exactly the named capabilities and
    the GLSL450 logical memory model."""
    if len(payload) % 4:
    ...
    while index < len(words):
        ...
        index += size
    if seen != capabilities or memory_model != (0, 1):
        raise ValueError("unexpected witness shader capabilities or memory model")
...
def main() -> None:
    ...
    build = ROOT / "build/t13-robustness2-witness"
    ...
    arrays = []
    shader_hashes = {}
    for name, (relative, capabilities) in SHADERS.items():
        target = build / f"{name}.spv"
        run(glslang, "-V", "--target-env", "vulkan1.0", str(ROOT / relative),
            "-o", str(target))
        payload = target.read_bytes()
        checked_spirv(payload, capabilities)
        arrays.append(emit_array(name, payload))
        shader_hashes[name] = hashlib.sha256(payload).hexdigest()
    (build / "t13_robustness2_shaders.h").write_text(
        "#include <stdint.h>\n" + "\n".join(arrays), encoding="utf-8")

    sdk_env = dict(os.environ, PS5_PAYLOAD_SDK=str(sdk), **SDK_SWITCHES)
    run(sys.executable, str(ROOT / "tools/build_sdk.py"), env=sdk_env)
    staged = ROOT / "dist-sdk"
    source = ROOT / "examples/t13_robustness2_witness/main.c"
    obj = build / "main.o"
    dep = build / "main.d"
    ...
    param.update(titleId="PPSA99994", conceptId="99994",
                 contentId="UP9000-PPSA99994_00-PS5VKRB2W0000001")
    param["localizedParameters"]["en-US"]["titleName"] = (
        "PS5 Vulkan Robustness2 Witness")
    ...
    artifact = {
        "profile": PROFILE,
        "storage_range": STORAGE_RANGE, "uniform_range": UNIFORM_RANGE,
        "sdk_switches": SDK_SWITCHES,
