...
def main() -> int:
    ...
    for profile, dump in dumps.items():
        for name, value in sorted(dump["features"].items()):
            if value:
                verdict, detail = "violation", "a feature bit is advertised"
            elif name in FEATURE_GATES:
                where, token, reason = FEATURE_GATES[name]
                path = ROOT / where
                if not path.is_file() or token not in path.read_text():
                    verdict = "not-audited"
                    detail = f"citation missing: {where} no longer contains {token!r}"
                else:
                    verdict = "satisfied"
                    detail = f"{reason} ({where})"
            elif name in FEATURE_ABSENT_FORMAT_FAMILY:
                where, tokens = FEATURE_ABSENT_FORMAT_FAMILY[name]
                text = (ROOT / where).read_text()
                present = [token for token in tokens if token in text]
                if present:
                    verdict = "violation"
                    detail = f"{where} mentions {present}, so the family may exist"
                else:
                    verdict = "satisfied"
                    detail = f"no {tokens[0]} family in {where}"
            elif name in FEATURE_WITHOUT_FRONTEND_GATE:
                verdict = "not-audited"
                detail = ("no frontend rejection branch; the dependent usage is an "
                          "application-side valid-usage rule or compiler-side")
            else:
                verdict = "satisfied"
                detail = "not required by the implemented profile"
    ...
    for row in features:
        row["applicable_cts"] = {"cases": [], "status": "not-selected",
                                 "note": "no selected case requires this bit"}
    for row in shaders:
        row["applicable_cts"] = {"cases": [], "status": "not-selected",
