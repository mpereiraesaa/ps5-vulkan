#!/usr/bin/env python3
"""Build the bounded SDK compute subgroup BASIC diagnostic witness."""
...
def checked_spirv(payload: bytes) -> None:
    ...
    while index < len(words):
        ...
        index += size
    if (capabilities != {1, 61} or subgroup_ops != [333, 333] or
            entry_models != [5]):
...
def main() -> None:
    ...
    shader_source = ROOT / "experiments/compute/t08_subgroup_basic_runtime.comp"
    shader_file = build / "basic.spv"
    run(glslang, "-V", "--target-env", "vulkan1.1", str(shader_source),
        "-o", str(shader_file))
    shader = shader_file.read_bytes()
    checked_spirv(shader)
    (build / "t08_subgroup_basic_shader.h").write_text(
        "#include <stdint.h>\n" + emit_array("t08_subgroup_basic_spirv", shader),
        encoding="utf-8")
    ...
    param.update(titleId="PPSA99994", conceptId="99994",
                 contentId="UP9000-PPSA99994_00-PS5VKSGBA0000001")
    param["localizedParameters"]["en-US"]["titleName"] = (
        "PS5 Vulkan Subgroup Basic Witness")
    ...
    artifact = {
        "profile": "t08-subgroup-basic-diagnostic-witness",
        "operation": operation,
        "groups": 2, "local_size": [16, 4], "subgroups": 4, "fields": 7,
