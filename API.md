...
- Render-pass `pNext`: exactly one `VkRenderPassMultiviewCreateInfo` is understood. It is parsed, validated and owned by the pass - the structure's implicit `sType` obligation is enforced, view-mask and view-offset counts must match the pass, the masks are all zero or all non-zero, a non-zero view offset requires a view-local dependency between two different subpasses, correlation masks must be disjoint, and anything chained behind it or duplicated stays fail-closed. Correlation masks are performance hints: they are validated and stored, never executed. This profile advertises no multiview feature and reports a zero `maxMultiviewViewCount`, so the only view masks it can accept are all zero, which is multiview disabled; enabling the feature is a later slice with its own evidence.
