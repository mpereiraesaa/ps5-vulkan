...
int main(void)
{
    ...
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_B8G8R8A8_UNORM,
        color.layout,VK_FALSE,VK_FALSE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_SUCCESS &&
    ...
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_B8G8R8A8_UNORM,
        color.layout,VK_FALSE,VK_FALSE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_ERROR_FEATURE_NOT_PRESENT);
    /* A colour attachment that is also read back ends the pass in the
     * transfer-source layout - that pass IS the handover to the readback the
     * pinned render-pass module records - and only that role may. */
    attachments[0].format=VK_FORMAT_R8G8B8A8_UNORM;
    attachments[0].initialLayout=VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    attachments[0].finalLayout=VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_R8G8B8A8_UNORM,
        color.layout,VK_FALSE,VK_TRUE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_SUCCESS);
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_R8G8B8A8_UNORM,
        color.layout,VK_FALSE,VK_FALSE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_ERROR_FEATURE_NOT_PRESENT);
    /* A depth attachment never ends in the colour readback's layout. */
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_R8G8B8A8_UNORM,
        color.layout,VK_TRUE,VK_TRUE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_ERROR_FEATURE_NOT_PRESENT);
    attachments[0].format=VK_FORMAT_B8G8R8A8_UNORM;
    attachments[0].finalLayout=VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    ...
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_B8G8R8A8_UNORM,
        color.layout,VK_FALSE,VK_FALSE,VK_SAMPLE_COUNT_1_BIT,&plan)==VK_ERROR_FEATURE_NOT_PRESENT);
    assert(ps5vk_attachment_plan(&attachments[0],VK_FORMAT_B8G8R8A8_UNORM,
        color.layout,VK_FALSE,VK_FALSE,VK_SAMPLE_COUNT_4_BIT,&plan)==VK_SUCCESS && !plan.clear);
    /* The depth role never carries more than one sample. */
    attachments[0].samples=VK_SAMPLE_COUNT_1_BIT;
    attachments[1].samples=VK_SAMPLE_COUNT_4_BIT;
    assert(ps5vk_attachment_plan(&attachments[1],VK_FORMAT_D32_SFLOAT,
        depth.layout,VK_TRUE,VK_FALSE,VK_SAMPLE_COUNT_4_BIT,&plan)==VK_ERROR_FEATURE_NOT_PRESENT);
    attachments[1].samples=VK_SAMPLE_COUNT_1_BIT;
