...
class VertexFormatVerifier(unittest.TestCase):
    def fixture(self):
        ...
        for case, (name, format_number, numeric, components) in enumerate(CASES):
            ...
            messages += [
                f"PS5VK_VERTEX_FORMAT_INPUT case={case} name={name} format={format_number} numeric={numeric} components={components} stride={int(components)*4} word=ffffffff",
        ...
        artifact = {"stage": "graphics-api-native-presentation-reuse", "scissor_probe": 8,
                    "geometry_fixture": "integer-vertex-formats", "compiler": "runtime-psbc-aco",
                    "graphics_shader_source": "owned-runtime-vertex-formats",
                    "runtime_graphics_inputs": {"vertex_sint": {}, "vertex_uint": {}, "fragment": {}},
    ...
    def test_rejects_wrong_component_completion(self):
        ...
        with self.assertRaisesRegex(ValueError, "GPU integer vertex-format oracle"):
