                        ...
                        struct ps5vk_graphics_module_key *out)
{
    ...
    if(clip_length || cull_length) {
        ...
        words[at++]=(4u<<16)|32u;words[at++]=5u;words[at++]=3u;words[at++]=4u; /* OpTypePointer */
        words[at++]=(4u<<16)|59u;words[at++]=5u;words[at++]=6u;words[at++]=3u; /* OpVariable %6 */
        if(cull_length)
            { words[at++]=(4u<<16)|59u;words[at++]=5u;words[at++]=7u;words[at++]=3u; }
    }
...
int main(void)
{
    ...
    /* A built-in the profile does not deliver for this storage class, and the
     * same declaration in the fragment stage, are both refused. */
    ...
    build_module(swapped_words,2,2,0,3,3,4,1,&declares_in_fragment);
    ...
    puts("Graphics stages: clip/cull declarations bounded, fragments refused, unusable modules report no counts");
    return 0;
}
