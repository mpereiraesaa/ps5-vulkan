...
evidence is a blocker. The checked matrix currently records 19/62 satisfied
...
`sampleRateShading`, `uniformBufferStandardLayout` and base `vulkanMemoryModel`
through their Vulkan 1.0 KHR routes, and the four rasterization and viewport features
`depthClamp`, `depthBiasClamp`, `fillModeNonSolid` and `multiViewport`) and 43
blockers.
