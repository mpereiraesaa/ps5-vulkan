...
check-sanitize: check-thread-sanitize
	...
	$(CC) -std=c11 -g -Wall -Wextra -Werror -fsanitize=address,undefined -fno-omit-frame-pointer $(VULKAN_CFLAGS) -Isrc $(VK_QUEUE_SOURCES) tests/test_vk_sync2.c -o build/tests/test_vk_sync2_sanitized
	./build/tests/test_vk_queue_sanitized
	./build/tests/test_vk_sync2_sanitized
	$(CC) -std=c11 -g -Wall -Wextra -Werror -fsanitize=address,undefined -fno-omit-frame-pointer $(VULKAN_CFLAGS) -Isrc $(VK_QUEUE_SOURCES) tests/test_coherent_memory.c -o build/tests/test_coherent_memory_sanitized
	./build/tests/test_coherent_memory_sanitized
...
check:
	...
	$(CC) -std=c11 -Wall -Wextra -Werror $(VULKAN_CFLAGS) -Isrc $(VK_PIPELINE_SOURCES) src/color_attachment_contract.c src/vk_graphics_pipeline.c src/vk_transform_feedback.c src/graphics_program.c tests/test_vk_graphics_pipeline.c -o build/tests/test_vk_graphics_pipeline
	./build/tests/test_vk_graphics_pipeline
	$(CC) -std=c11 -Wall -Wextra -Werror $(VULKAN_CFLAGS) -Isrc src/vk_transform_feedback.c tests/test_vk_transform_feedback.c -o build/tests/test_vk_transform_feedback
	./build/tests/test_vk_transform_feedback
	...
	$(CC) -std=c11 -Wall -Wextra -Werror $(VULKAN_CFLAGS) -Isrc $(VK_DEVICE_SOURCES) tests/test_dxvk_sync2_negotiation.c -o build/tests/test_dxvk_sync2_negotiation
	./build/tests/test_dxvk_sync2_negotiation
	...
	$(CC) -std=c11 -Wall -Wextra -Werror $(VULKAN_CFLAGS) -Isrc $(VK_QUEUE_SOURCES) tests/test_coherent_memory.c -o build/tests/test_coherent_memory
	./build/tests/test_coherent_memory
...
graphics-stage-shaders:
	...
	$(GLSLANG) -V experiments/graphics/xfb_capture.vert -o build/runtime-graphics/xfb_capture.vert.spv
	$(GLSLANG) -V experiments/graphics/xfb_capture.frag -o build/runtime-graphics/xfb_capture.frag.spv
	$(GLSLANG) -V -S geom experiments/graphics/xfb_capture.geom -o build/runtime-graphics/xfb_capture.geom.spv
	$(GLSLANG) -V -S geom experiments/graphics/xfb_streams.geom -o build/runtime-graphics/xfb_streams.geom.spv
